        ...
        <CardContent>
          <div className="rounded-md border border-border/50 overflow-hidden">
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">Loading tasks...</TableCell></TableRow>
                ) : jobs?.length === 0 ? (
                  <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No active jobs.</TableCell></TableRow>
                ) : jobs?.map((job: any) => (
                  <TableRow key={job.id} className="border-border/50 hover:bg-white/[0.02]">
                    <TableCell className="font-medium">{job.task}</TableCell>
                    <TableCell className="font-numeric text-muted-foreground">#{job.botId}</TableCell>
